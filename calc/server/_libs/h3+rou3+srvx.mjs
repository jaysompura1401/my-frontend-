...
var gracefulShutdownPlugin = (server) => {
	...
	if (!globalThis.process?.on || config === false || config === void 0 && (processModule.env.CI || processModule.env.TEST)) return;
	const gracefulTimeout = config === true || !config?.gracefulTimeout ? Number.parseInt(processModule.env.SERVER_SHUTDOWN_TIMEOUT || "") || 5 : config.gracefulTimeout;
	let isClosing = false;
	let isClosed = false;
	const w = server.options.silent ? () => {} : processModule.stderr.write.bind(processModule.stderr);
...
//#endregion
export { FastResponse as a, gracefulShutdownPlugin as c, toRequest as i, wrapFetch as l, HTTPError as n, FastURL as o, defineLazyEventHandler as r, errorPlugin as s, H3Core as t };
